function sift_arr = sp_find_lbpsift_grid(I, grid_x, grid_y, patch_size, sigma_edge,result,mapping)
...
sift_arr = zeros(num_patches, num_samples * num_angles+num_samples * mapping.num);
...
interval = interval - (1/num_bins + 1);%from -1 to 1,num_bins numbers
...
% for all patches num_patches = numel(grid_x)
for i=1:num_patches
    r = patch_size/2;
    cx = grid_x(i) + r - 0.5;%center x
    cy = grid_y(i) + r - 0.5;%center y

    % find coordinates of sample points (bin centers)
    sample_x_t = sample_x * r + cx; %bin centers
    sample_y_t = sample_y * r + cy; %bin centers
    ...
    sift_arr(i,1:num_samples * num_angles) = reshape(curr_sift, [1 num_samples * num_angles]);    
        
end

[m,n]=size(grid_x);
pace=patch_size/num_bins;
loop=0;
for i=1:n
    for j=1:m
    loop=loop+1;
    temp=result(grid_y(j,i):grid_y(j,i) + patch_size-1,grid_x(j,i):grid_x(j,i) + patch_size-1);
    temp2=[];
%     figure;
%     subplot(3,1,1);imshow(temp);
%     subplot(3,1,2);imshow(I(gridY(j,i):gridY(j,i) + patchSize-1,gridX(j,i):gridX(j,i) + patchSize-1));
    for ii=1:num_bins
        for jj=1:num_bins
            temp1=temp((ii-1)*pace+1:ii*pace,(jj-1)*pace+1:jj*pace);
            x=hist(temp1(:),0:(mapping.num-1));           
            temp2=[temp2,x];
        end
    end
    sift_arr(loop,num_samples * num_angles+1:end)=temp2; 
    sift_arr(loop,num_samples * num_angles+1:end)=sift_arr(loop,num_samples * num_angles+1:end)/max(sift_arr(loop,num_samples * num_angles+1:end))/5; 
%     subplot(3,1,3);stem(sift_arr(loop,:));
    end
end
